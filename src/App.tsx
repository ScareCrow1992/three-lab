...
function SideTab({ id, label, selected }: { id: string, label: string, selected : boolean }) {
  ...
  return (
    <>
      ...
      [&::-webkit-scrollbar-thumb]:rounded-full">
        ...
        {TABS.map((t) => (
          <li key={t.id}>
            <SideTab id={t.id} label={t.label} selected={t.id === selected_tab} />
          </li>
        ))}
      </ul>
    </>)
...
function App() {
  ...
  return (
    <>
      <div className="flex">
        <div className="bg-[#282E38] h-screen relative">
          <SideTabNav />
